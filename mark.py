...
def run_rake(text):
	return [item[0] for item in rake_object.run(text) if ' ' not in item[0] and '\n' not in item[0]]
...
class Person:
	...
	def return_statement(self, text):
		text = [item for item in text.split(' ') if item in self.topics and len(self.topics[item])]
		if (len(text) == 0):
...
def convo(first, second, start_text, length = 20):
	...
	while (first_response != 'I have nothing to say' and second_response != 'I have nothing to say'):
